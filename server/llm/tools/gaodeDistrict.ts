 ...
 * 用于查询省市区县级行政区划信息
 */
export class GaodeDistrictTool implements BaseTool {
    readonly name = 'gaode_district';
    readonly description = '查询中国行政区划信息，支持按城市名称或 adcode 查询省市区县等各级行政区域详情。';
    readonly schema: Array<{ name: string; type: 'string' | 'number' | 'boolean' | 'object' | 'array'; description?: string; required?: boolean }> = [
        {
            name: 'keywords',
            type: 'string',
            description: '查询关键字，可以是行政区名称（如"北京"）、citycode 或 adcode',
            required: true
        },
        {
            name: 'subdistrict',
            type: 'number',
            description: '返回下级行政区级数：0-不返回，1-下一级，2-下两级，默认 1',
            required: false
        },
        {
            name: 'extensions',
            type: 'string',
            description: '返回结果控制："base"-不返回边界坐标，"all"-返回当前 district 边界值',
            required: false
        }
    ];
 ...
 * 创建高德行政区域查询工具实例
 */
export function createGaodeDistrictTool(apiKey: string): GaodeDistrictTool {
    return new GaodeDistrictTool(apiKey);
}
